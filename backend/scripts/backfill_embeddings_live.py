...
def main() -> None:
    ...
    parser.add_argument("--since_days", type=int, default=7, help="only backfill articles published within last N days")
    parser.add_argument("--limit", type=int, default=300, help="max number of articles to backfill per run")
    ...
    print(f"[db] rows to embed: {len(target_ids)} (since_days={args.since_days}, limit={args.limit})")
